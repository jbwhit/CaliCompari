#!/home/ssi/jwhitmore/progs/epd-7.1-2-rh5-x86_64/bin/python#!/home/ssi/jwhitmore/progs/epd-7.1-2-rh5-x86_64/bin/python
# 2011-11-16 objectoriented.py
...
class Exposure(object):
  ...
  def __init__(self, arcFile='', reductionProgram='', calibrationType='', calibrationFile='', exposureFile=''):
    ...
    self.fitGuess.update({ 'fsigma':90.5, 'fix_fsigma':False, 'limit_fsigma':(5,2000) ,'err_fsigma':5 })
    self.fitGuess.update({ 'fmultiple':1.25, 'fix_fmultiple':False, 'limit_fmultiple':(0.1, 5.0) ,'err_fmultiple':0.2 })
    self.fitGuess.update({ 'fslope':0.005, 'fix_fslope':False, 'limit_fslope':(-1.0,1.0) ,'err_fslope':0.05 })
    self.fitGuess.update({ 'elements':1000, 'fix_elements':True })
  ...
  def fitOrderShift(self, order, fmultiple, fshift, fsigma, elements, **kwargs):
    """docstring for fitOrderShift"""
    kernel = self.gaussKernel(self.fitGuess['elements'], fsigma)
    tck = si.splrep(self.Orders[order]['oiow'], np.convolve(kernel, self.Orders[order]['oiof'] * fmultiple, mode='same'))
    # overflx = np.average(si.splev(np.hstack(self.Orders[order]['overwav']) + fshift, tck).reshape(np.shape(self.Orders[order]['overwav'])), axis=1)
